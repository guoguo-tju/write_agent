...
- 新增公众号排版页 `/layout`：支持从改写/审核/封面页一键跳转并携带 `rewrite_id` 自动导入正文与封面，提供主题切换、实时预览与一键复制公众号。

## 主要流程与页面截图

1. **改写**：输入原文、选择风格、设置目标长度（预设 `100-8000`，默认 `500`）并流式输出。顶部导航支持全站 `CN / EN` 切换，选择会持久化到 `localStorage`（`write_agent_lang`）。
