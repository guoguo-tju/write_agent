...
- 新增全站中英文切换（`CN / EN`），默认中文，语言选择持久化到 `localStorage`（`write_agent_lang`）。
- 新增 **排版** 页面（`/layout`），支持公众号排版主题切换、实时预览、富文本/图片粘贴转 Markdown、公众号兼容复制。
- 在改写/审核/封面页新增“去排版”入口；跳转自动携带 `rewrite_id`，并自动导入正文（有封面时会置顶插入封面图）。
- 改写目标长度档位扩展为 `[100, 300, 500, 800, 1000, 1500, 2000, 5000, 8000]`，默认值 `500`。
- 前端针对 `/layout` 完成分包优化（路由懒加载 + manual chunks + `highlight.js` 按需语言），降低首屏与排版页加载压力。
